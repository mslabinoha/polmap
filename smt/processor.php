...
if(!isset($_SESSION['session_username'])){header( "Location: /log/login.php" );}else{
    ...
    if (!empty($_FILES['imgupload']['name'])){
        ...
        $result = mysql_query ("INSERT INTO smimages(path,data,authid,smitnid) VALUES('".$imgpath."',CURDATE(),".$_SESSION['session_userid'].",".$_POST['tid'].");"); //прописуємо в бд
    }
    ...
    if (!empty($_POST["commenta"])){
        echo $_POST["commenta"];
        $result = mysql_query ("INSERT INTO smcomments(ctext,data,authid,smitnid) VALUES('".$_POST["commenta"]."',CURDATE(),".$_SESSION['session_userid'].",".$_POST['tid'].");");
    }
    $url='/smt/trashdetails.php?tid='.$_POST['tid'];
    header( "Location: $url" );
}
?>